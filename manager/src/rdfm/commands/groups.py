...
def list_groups(config: rdfm.config.Config, args) -> Optional[str]:
    ...
    for group in groups:
        ...
        print(
            f"\tDevices assigned: {group.devices} | {len(group.devices)} "
            "devices"
        )
...
def assign_package(config: rdfm.config.Config, args) -> Optional[str]:
    ...
    packages = (
        [] if args.package_id is None else [int(x) for x in args.package_id]
    )
    return rdfm.api.groups.assign(config, args.group_id, packages)
...
def modify_devices(config: rdfm.config.Config, args) -> Optional[str]:
    ...
    return rdfm.api.groups.assign_device(
        config, args.group_id, insertions, removals
    )
...
def add_groups_parser(parser: argparse._SubParsersAction):
    ...
    create.add_argument(
        "--name", type=str, required=True, help="name of the group"
    )
    create.add_argument(
    ...
    create.add_argument(
        ...
        type=str,
        action="append",
        help="append extra metadata to the group, specified as "
        "key=value pairs, can be provided multiple times",
    )
    ...
    target = sub.add_parser(
        "target-version",
        help="set the target version the group should be updated to",
    )
    ...
    target.add_argument(
        "--version", type=str, required=True, help="version identifier"
    )

    assign = sub.add_parser(
        "assign-package", help="assign packages to a group"
    )
    ...
    assign.add_argument(
        ...
        type=str,
        action="append",
        help="package identifiers to assign, which will replace all previous "
        "assignments. Can be provided multiple times (or ignored entirely to "
        "clear all assignments)",
    )

    modify = sub.add_parser(
        "modify-devices", help="modify devices belonging to a group"
    )
    ...
    modify.add_argument(
        ...
        type=str,
        action="append",
        help="device identifiers of devices to add to the group, can be "
    ...
    modify.add_argument(
        ...
        type=str,
        action="append",
        help="device identifiers of devices to remove from the group, can be "
        "provided multiple times",
    )
