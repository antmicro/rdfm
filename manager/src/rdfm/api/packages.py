...
def fetch_all(config: rdfm.config.Config) -> List[Package]:
    response = requests.get(
        rdfm.api.escape(config, "/api/v1/packages"),
...
def upload(
    ...
    monitor = requests_toolbelt.MultipartEncoderMonitor(
        encoder, lambda monitor: progress(monitor.bytes_read, monitor.len)
    )
...
def delete(config: rdfm.config.Config, package_id: int) -> Optional[str]:
    ...
    response = requests.delete(
        rdfm.api.escape(config, f"/api/v1/packages/{package_id}"),
        verify=config.ca_cert,
        auth=config.authorizer,
    )
    if response.status_code == 409:
        return (
            "Deleting package failed: Package is assigned to at least "
            "one group and cannot be removed"
        )
