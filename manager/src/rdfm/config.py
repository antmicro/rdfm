...
class Config:
    ...
    Example: https://rdfm.example.com/ https://rdfm.example.com:5000/
    """
    server_url: str
...
def try_get_config(
    dict: dict[str, typing.Any], key: str, help_text: str
    ...
    if key not in dict:
        print(
            "Configuration file is missing a setting:", key, f"({help_text})"
        )
        return None
    return dict[key]
...
def load_auth_from_file(config: Config, path: str = MANAGER_CONFIG_DEFAULT):
    ...
    if not os.path.isfile(expanded_path):
        raise RuntimeError(
                "Authorization is enabled, but config file does not exist: "
                f"{expanded_path}"
        )
    ...
    try:
        ...
        auth_url: typing.Optional[str] = try_get_config(
            values, "auth_url", "Authorization server URL"
        )
        client_id: typing.Optional[str] = try_get_config(
            values, "client_id", "Authorization Client Identifier"
        )
        client_secret: typing.Optional[str] = try_get_config(
            values, "client_secret", "Authorization Client Secret"
        )
