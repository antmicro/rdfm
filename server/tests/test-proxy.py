...
def not_encrypted_proxy():
    child_server = pexpect.spawn('bash -c "python3 -m rdfm_mgmt_server -no_ssl 2>&1 | tee proxy-noenc-server.log"')
    child_server.expect("Listening for connections on 127.0.0.1:1234...")

    time.sleep(5)
    child_user = pexpect.spawn('bash -c "python3 -m rdfm_mgmt_client u -no_ssl 2>&1 | tee proxy-noenc-manager.log""')
...
def encrypted_proxy():
    child_server = pexpect.spawn('bash -c "python3 -m rdfm_mgmt_server 2>&1 | tee proxy-enc-server.log"')
    child_server.expect("Listening for connections on 127.0.0.1:1234...")
...
def two_proxies():
    child_server = pexpect.spawn('bash -c "python3 -m rdfm_mgmt_server -no_ssl 2>&1 | tee 2proxy-server.log"')
    child_server.expect("Listening for connections on 127.0.0.1:1234...")

    time.sleep(5)
    child_user = pexpect.spawn('bash -c "python3 -m rdfm_mgmt_client u -no_ssl 2>&1 | tee 2proxy-manager.log"')
    child_device = pexpect.spawn(
