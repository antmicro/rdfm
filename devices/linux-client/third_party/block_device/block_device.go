...
func (bd bdevice) Open(device string, size int64) (*BlockDevice, error) {
	...
	if mntPt := checkMounted(device); mntPt != "" {
		log.Warnf("Inactive partition %q is mounted at %q. "+
			...
			"RDFM to avoid any issues.", device, mntPt)
	...
	log.Infof("Native sector size of block device %s is %v bytes. RDFM will write in chunks of %v bytes",
		device,
		nativeSsz,
		chunkSize,
	)
